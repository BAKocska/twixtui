...
// stamp identifies the version of a file that a store has in memory. Reads
// stat the file and reload only when the stamp has moved, so a long-lived UI
// picks up another process's writes without re-reading on every render.
//
...
// directory takes effect all at once, so a crash or a full disk can never
// leave a half-written file where the store expects a whole one. What each
