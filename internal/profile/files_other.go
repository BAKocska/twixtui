...
// openRead opens a stored file for reading. Everywhere but Windows an open file
// is no obstacle to replacing it: the reader keeps the file it opened and the
// next open finds the new one.
func openRead(path string) (*os.File, error) { return os.Open(path) }

// readWholeFile returns the contents of a stored file.
