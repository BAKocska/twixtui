...
// another user who let this one look at it, and reading the standings there is a
...
// lock file refuses it the write too. An exclusive lock is never softened, so
// recording a result on an unwritable directory still fails, and concurrent
