// animations-v3.jsx — the continuous-composition animation engine this demo is
// drawn with, vendored into the repo. Raw elements, hex colours and pixel
// units are deliberate here.
...
//   WATERCOLOR (unused by this demo; the kit it needs is not vendored here,
//   and every component below degrades to a placeholder without it).
//   A painting is a function(p) written
...
function Stage({
  ...
  React.useEffect(() => {
    ...
    return () => {
      ...
      // (a document edit can retime mid-host-play, no gesture involved) and
...
function PlaybackBar({ time, duration, playing, onPlayPause, onReset, onSeek, onHover }) {
  ...
  return (
    <div data-preview-chrome style={{
      // Slimmed to the single-scrubber look: transport first, tighter
      // metrics, quieter chrome. It is preview furniture, outside the
      // exportable svg, so it never reaches an exported frame; a timeline
      // host with its own transport suppresses it.
      ...
      {/* No export control here: exporting is the render script's job, and it
          drives the stage's own data-om-seek-to-time-frame transport. A
          button in the preview bar could only ask a host frame that is not
          there. */}
    </div>
  );
}
...
// shape rules and constants (16KB raw cap, 50 entries, dur finite in
